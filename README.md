...
```
 raw syslog/JSON/CSV  ──▶  classify  ──▶  zero-copy parse  ──▶  OCSF 1.3 JSON
        │                                                            │
        │            SHA-256(raw) · UUIDv7 event_id                  │
        └─────────────────────▶  RFC 6962 Merkle root  ──▶  ledger.jsonl
                                                              │
                                              Parquet WORM block + verify (0/1/2)
```
...
```
                       ┌────────────────────────────────────────────────┐
 UDP/TCP syslog ──────▶│  Tier-1  Signature LRU Cache                   │
 (SO_REUSEPORT,         │  16 shards × 512 entries (8,192), FxHash      │──▶ hit: sub-µs path
  tokio workers)        ├────────────────────────────────────────────────┤
                        │  Tier-2  Drain template miner                 │──▶ miss: line-rate
                        │  prefix tree + anchor tokens (ALLOW/DENY,     │    clustering,
                        │  message codes, class keys — never wildcarded)│    on miss only
                        ├────────────────────────────────────────────────┤
                        │  Tier-3  Laya decision engine                 │──▶ novel clusters
                        │  bounded crossbeam ring (10,000) — NEVER      │    triaged out-of-
                        │  blocks the ingest hot path (invariant #5)    │    band, zero
                        └────────────────────────────────────────────────┘    hallucination
                                                       │
                 OCSF 1.3 event ◀── zero-copy vendor extractors (cisco_asa, fortigate,
                        │           paloalto, pfsense, suricata, cef — byte slices)
                        ▼
        batcher (dual trigger: 1,000 events / 2,000 ms)
                        │
        SHA-256(raw) + UUIDv7 + RFC 6962 Merkle leaf ──▶ ledger.jsonl (append-only)
                        │
        Parquet WORM block (raw_log kept byte-for-byte) ──▶ ulpf verify → 0 / 1 / 2
```

**Why two engines, not one?** The frozen Aho-Corasick **baseline** (`UniversalParser`) is the scientific control: the 3-tier pipeline must *strictly improve* on it under identical corpora. Every scorecard in §4–§5 shows both columns — no self-graded single numbers.
...
| b | Extract and parse source-specific attributes | ✅ | zero-copy extractors (ASA/FortiGate/PAN-OS/pfSense/Suricata/CEF) — **Macro F1 100%** core &amp; full (§4.1–4.2) |
...
**Performance gates** (checked whenever hot path/miner changes): p50 &lt; 5.0 µs · LRU hit rate &gt; 90% · Action Inviolability 100% · grouping accuracy &gt; 90%.
