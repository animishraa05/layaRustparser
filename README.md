...
> **Theme:** Blockchain &amp; Cybersecurity · **Category:** Software · **Requirement (k):** standalone binary &lt; 35 MB ✅ (18.6 MB / 17.7 MiB)
...
- **Zero-copy hot path** — p50 **3.02 µs**/line on the core corpus and **7.28 µs** at 224k lines (**−96.2% / −99.3%** vs the frozen baseline); **849,481 EPS** at full scale (**2.15×** baseline).
...
- **Lossless provenance** — raw bytes stored byte-for-byte (`raw_log`), `raw_hash = SHA-256(raw)` verified on **all 224,657** full-scale lines, UUIDv7 `event_id` per event.
- **RFC 6962 Merkle WORM** — append-only `ledger.jsonl`; change one byte and `ulpf verify` exits **2**; the live run verified **186/186** blocks PASS.
- **One schema out: OCSF 1.3** — `NetworkActivity` (class 4001) across **6 formats today**: Cisco ASA · FortiGate · PAN-OS · pfSense · Suricata · CEF (matrix in §2).
- **Action inviolability 100%** — `ALLOW`/`PERMIT`/`ACCEPT` can never merge into the same template cluster as `DENY`/`DROP`/`BLOCK`/`REJECT`.
- **4,312× template compression** — 224,657 lines → **32** Drain templates (baseline: 137,986) with template accuracy held at 100% — ready-made features for SIEM/ML.
- **Truly air-gapped** — zero outbound calls, no telemetry or model downloads; a single static **18.6 MB** binary (requirement: &lt; 35 MB) plus Docker.

**Proof is checked into this repo.** Every accuracy number below links to a timestamped evaluation report generated by `ulpf evaluate`; every table can be regenerated with one command (see [§7 Reproduce the proof](#7-reproduce-the-proof)).
...
How the incumbent shippers handle exactly these axes — architectural facts from their own public documentation, **not** benchmarks (our measured numbers are all in §1):

| | Runtime footprint | Normalization | Stream-level provenance | Air-gapped install |
| :--- | :--- | :--- | :---: | :---: |
| **Logstash** (Elastic, JVM) | JVM heap — typically hundreds of MB to GB | hand-written `grok` patterns | ❌ none | ❌ plugin installs pull from the internet |
| **Fluentd** | Ruby + native C, ~100 MB class | hand-written filter plugins | ❌ none | ❌ `gem install` pulls from the internet |
| **Splunk Universal Forwarder** | proprietary agent | Splunk CIM, paid license | ❌ none | 🟡 offline package, licensed |
| **ULPF** (this repo) | single **18.6 MB** static Rust binary | OCSF 1.3, automatic (zero-copy extractors + Drain) | ✅ RFC 6962 Merkle root → append-only ledger → Parquet WORM, exit-code audit | ✅ zero network calls by design |
...
| :--- | :--- | :--- | :---: |
| RFC 3164 syslog — **Cisco ASA** | `%ASA-` | `cisco_asa.rs` | ✅ today |
| key-value syslog — **FortiGate** | `devname="` · `type="traffic"` · `logid="` | `fortigate.rs` | ✅ today |
| CSV syslog — **Palo Alto PAN-OS** | `,TRAFFIC,` · `,THREAT,` · `,SYSTEM,` · `PAN-OS` | `paloalto.rs` | ✅ today |
| EVE JSON — **Suricata** | `{"timestamp":` · `"event_type":` · `"flow":` · `"alert":` | `suricata.rs` | ✅ today |
| filterlog — **pfSense** | `filterlog[` · `filterlog:` | `pfsense.rs` | ✅ today |
| **CEF** envelope (vendor-neutral) | `CEF:` | `cef.rs` | ✅ today |
| LEEF · generic `key=value` · flat JSON · XML · RFC 5424 (structured data) | — | — | 🟡 P10.1 (§16) |
| anything else | — | generic fallback event — vendor `Unknown`, **never silently dropped** | ⚪ by design |

~10 additional vendors (ISRO-relevant) are P10.2 (§16) — and you don't need to wait for code: `ulpf onboard` synthesises and validates a parser from 3–5 sample lines, fully offline (§11, step 7).

### One real line, end to end

Every number in this README traces back to records like this one — verbatim from [`data/raw/cisco_asa.log`](data/raw/cisco_asa.log) (line 218):

**Input** — these exact bytes are stored forever in the `raw_log` column:
...
**Output** — the actual stored OCSF event from `data/parquet/block_00001.parquet` (stored compact in the `ocsf_json` column; pretty-printed here):
...
- `metadata.raw_data` **is** the input byte-for-byte, and `metadata.raw_hash` is its SHA-256 — verify it yourself: `printf '%s' '<input above>' | sha256sum` → `167c1cec…55fd`, match.
- `%ASA-6-302013` (“Built”) becomes OCSF vocabulary: `disposition: "Allowed"`, `activity_name: "Open"`, endpoints + direction in canonical fields — while vendor-specific leftovers (`connection_id`, `cisco_message_code`) are kept under `unmapped`, not thrown away.
...
**Why two engines, not one?** The frozen Aho-Corasick **baseline** (`UniversalParser`) is the scientific control: the 3-tier pipeline must *strictly improve on latency and accuracy at every scale, and on throughput at scale* under identical corpora — **2.15×** EPS at 224k lines, with the honest small-corpus exception (0.82–0.96×, §14). Every scorecard in §4–§5 shows both columns — no self-graded single numbers.
...
Nothing below is graded without an exact ruler — definition, code pointer (all in [`crates/ulpf-ai/src/evaluator.rs`](crates/ulpf-ai/src/evaluator.rs)), and ground-truth source:
...
**Ground-truth policy:** sidecar GT is *authoritative* over in-line derivation (`:1693–1703`) — fuzz mutations destroy in-line markers, so on the adversarial/holdout corpora the sidecar is the only honest ruler. The core corpus (1,720 lines) carries no sidecar: its GT is derived in-line from the lines themselves. Sidecars exist for adversarial (757), holdout (200) and full-scale (224,657 — the 1,000,000-correct-field table in §1).

† **Naming caveat, stated plainly:** despite the label *“Macro F1”*, this metric is **not** a precision/recall F1. It is the macro-average of five exact-match field accuracies (`evaluator.rs:1970`). Read it everywhere in this repo — README, reports, matrix — as **mean field accuracy**; the historical name is kept only so the tables match the generated reports.
...
| Action Inviolability | N/A | **100% preserved** | ✅ |
...
From §1b of each report — these are *gates*, not decorations:
...
**Cryptographic chain of custody** — the wow factor:
...
> ⚠️ `evaluate`/`benchmark` must be **release** builds, and runs should be back-to-back on an otherwise idle machine: accuracy rows are deterministic (they came out byte-identical across every rerun today), but throughput/latency rows swing with system load — that's why the reports carry timestamps and why we discarded a racy parallel batch during today's measurement session.
...
| :--- | :--- | :---: | :--- |
| a | Preserve complete raw event data without information loss | ✅ | `raw_log` byte-exact + `raw_hash == SHA-256(raw)` on **all 224,657** full-scale lines (§6) |
| b | Extract and parse source-specific attributes | ✅ | zero-copy extractors (ASA/FortiGate/PAN-OS/pfSense/Suricata/CEF) — **mean field accuracy (“Macro F1” †) 100%** on core &amp; full (§4.1–4.2; rulers in §4) |
| c | Normalize fields into a common event taxonomy | ✅ | OCSF 1.3 `NetworkActivity` 4001 — **100% VCA** on core, adversarial-vendor-match, full |
| d | Maintain traceability between normalized and original events | ✅ | UUIDv7 `event_id` + SHA-256 digest on every event; `inspect` demo (§11.6) |
| e | Plug-and-play onboarding of new log sources | ✅ | `ulpf onboard` — 3–5 sample lines → validated parser spec, zero network (§11.7) |
| f | Unified visibility across enterprise environments | ✅ | 5 vendor families → uniform OCSF JSON + Parquet schema (§13) |
| g | Efficient SIEM and Data Lake integration | ✅ | Parquet WORM blocks, queryable via DuckDB/pandas (§13 snippet) |
| h | AI/ML-ready security and operational analytics | ✅ | **32 Drain templates from 224,657 lines (4,312× compression)** — pre-clustered feature IDs (§5) |
| i | Reduced parser development effort | ✅ | sample file → parser spec in **ms**, not days (§11.7) |
| j | Deployable in an air-gapped network | ✅ | single self-contained binaries, **zero** outbound calls anywhere in the runtime path |
| k | Packaged in a container for platform independence (target &lt; 35 MB) | 🟡 | binary **18.6 MB ✅** (`ls -la target/release/ulpf`); container slim-down in progress — §16 P10.7 |
...
Measured, documented, not swept under the rug:
...
- **Full-scale p50 (7.28 µs) is over the 5.0 µs latency gate** — the gate is calibrated on the core corpus (3.02 µs ✅ passes); at 224,657 lines the working set no longer stays cache-resident and p50 lands at 7.28 µs (still −99.3% vs baseline). Tracked openly here rather than quietly re-tuning the gate (§16).
...
**Performance gates** (checked whenever hot path/miner changes): p50 &lt; 5.0 µs *(core-corpus ruler: 3.02 µs passes; full-scale 7.28 µs is currently over the line — tracked in §14)* · LRU hit rate &gt; 90% · Action Inviolability 100% · grouping accuracy &gt; 90%.
